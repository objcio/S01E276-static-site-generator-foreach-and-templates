...
struct Index: Rule {
    var body: some Rule {
        Write(contents: h1 { "Homepage" }, to: "index.html")
    }
}
...
struct SiteTemplate: Template {
    func apply(environment: EnvironmentValues, content: Node) -> Node {
        HTML.html {
            head {
...
struct BlogTemplate: Template {
    @NodeBuilder func apply(environment: EnvironmentValues, content: Node) -> Node {
        h1 { "Blog" }
        content
    }
}
